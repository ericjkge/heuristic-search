...
def pack(n):
    ...
    # Multi-start search with stronger geometry-aware refinement.
    best = None
    seeds = [0, 1, 2, 3, 4, 5, 11, 17, 23, 29]
    modes = (0, 1, 2)
    ...
    for seed in seeds:
        ...
        for mode in modes:
            ...
            # Alternate between local perturbation and boundary tightening.
            for _ in range(3):
                centers, angles, _ = local_optimize(
                    centers, angles,
                    max_iter=2600 if n <= 12 else 1800,
                    seed=seed + 31 * mode
                )
                centers, angles, _ = refine_with_global_shifts(
                    centers, angles,
                    seed=seed + 31 * mode
                )
                centers = repair_scale(centers, angles)
    ...
    # Basin-hopping around the best candidate.
    centers, angles, s = best
    rng = random.Random(99991 + 13 * n)

    for _ in range(40 if n <= 12 else 24):
        c = centers[:]
        a = angles[:]

        # Perturb a small subset, with orientation flips encouraged.
        k = 1 if n <= 6 else (2 if n <= 12 else 3)
        idxs = [rng.randrange(n) for _ in range(k)]
        for idx in idxs:
            dx = (rng.random() * 2.0 - 1.0) * (0.12 if n <= 12 else 0.08)
            dy = (rng.random() * 2.0 - 1.0) * (0.12 if n <= 12 else 0.08)
            da = (rng.random() * 2.0 - 1.0) * 0.20
            if rng.random() < 0.45:
                da += math.pi
            c[idx] = (c[idx][0] + dx, c[idx][1] + dy)
            a[idx] = normalize_angle(a[idx] + da)
        ...
        c = repair_scale(c, a)
        c, a, _ = local_optimize(
            c, a,
            max_iter=1600 if n <= 12 else 1100,
            seed=17 + _ + n
        )
        c, a, _ = refine_with_global_shifts(c, a, seed=77 + _ + n)
        c = repair_scale(c, a)
        ss = enclosing_side(c, a)
        if ss < s:
            centers, angles, s = c, a, ss

    # Final targeted tightening: re-optimize only if the current layout has room.
    centers = repair_scale(centers, angles)
    centers, angles, _ = local_optimize(
        centers, angles,
        max_iter=2200 if n <= 12 else 1400,
        seed=999 + n
    )
    centers, angles, _ = refine_with_global_shifts(centers, angles, seed=1999 + n)
    centers = repair_scale(centers, angles)
    s = enclosing_side(centers, angles)
