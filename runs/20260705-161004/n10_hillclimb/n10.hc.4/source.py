...
def pack(n):
    ...
    # Build a stronger initial solution using several hand-structured motifs.
    best = None
    seeds = 18

    def make_seeded_layout(seed):
        rng = random.Random(1234567 + 97 * n + seed)
        angles = []
        centers = []

        # Use an approximate double-lattice orientation pattern, with occasional
        # third-state angles for boundary adaptation.
        base_angles = [math.pi / 2.0, -math.pi / 2.0]
        if n >= 7:
            base_angles += [math.pi / 10.0, -math.pi / 10.0]

        # Candidate row/column organizations.
        configs = []
        m = int(math.ceil(math.sqrt(n)))
        for cols in range(max(2, m - 3), m + 4):
            rows = int(math.ceil(n / cols))
            configs.append((cols, rows))
        configs.append((int(math.ceil(n / 3.0)), 3))
        configs.append((3, int(math.ceil(n / 3.0))))
        configs.append((int(math.ceil(n / 4.0)), 4))
        configs.append((4, int(math.ceil(n / 4.0))))

        cols, rows = configs[seed % len(configs)]

        # Better starting pitches than the old grid; search around these with
        # deterministic jitter.
        pitch_x0 = WIDTH * (0.58 + 0.05 * ((seed % 3) - 1))
        pitch_y0 = HEIGHT * (0.60 + 0.04 * (((seed // 3) % 3) - 1))

        for k in range(n):
            j, i = divmod(k, cols)
            x = (i - (cols - 1) / 2.0) * pitch_x0
            y = (j - (rows - 1) / 2.0) * pitch_y0

            # Stagger alternate rows to exploit pentagon concavities.
            if j % 2:
                x += 0.5 * pitch_x0
            if (i + j) % 3 == 1:
                y += 0.18 * pitch_y0

            # Subtle boundary-tilt style jitter.
            x += (rng.random() - 0.5) * 0.10
            y += (rng.random() - 0.5) * 0.10

            ang = base_angles[(i + 2 * j + seed) % len(base_angles)]
            ang += (rng.random() - 0.5) * 0.12

            centers.append((x, y))
            angles.append(ang)

        # Quick center bias toward the square center before repair.
        mx = sum(x for x, _ in centers) / n
        my = sum(y for _, y in centers) / n
        centers = [(x - mx, y - my) for x, y in centers]
        return centers, angles

    # Deterministic multi-start plus more aggressive local optimization.
    for seed in range(seeds):
        centers, angles = make_seeded_layout(seed)
        centers = repair_scale(centers, angles)
        s = enclosing_side(centers, angles)
        centers, angles, s = improve_by_local_search(
            centers,
            angles,
            steps=6000 if n >= 8 else 3500,
            seed=seed,
        )
        centers = repair_scale(centers, angles)
        s = enclosing_side(centers, angles)
    ...
    # A short polish phase starting from the current best.
    centers, angles, s = best
    for seed in range(6):
        centers2, angles2, s2 = improve_by_local_search(
            centers,
            angles,
            steps=1800,
            seed=1000 + seed,
