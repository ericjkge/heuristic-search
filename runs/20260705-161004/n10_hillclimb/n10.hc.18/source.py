...
def pack(n):
    ...
    def best_permutation_reseed(centers, angles, base_seed):
        """Cheap discrete improvement: reorder optimizers' focus and try angle flips."""
        rng = random.Random(base_seed)
        idx = list(range(len(centers)))
        rng.shuffle(idx)
        c2 = [centers[i] for i in idx]
        a2 = [angles[i] for i in idx]
        return c2, a2

    def anneal_with_wall_bias(centers, angles, seed, max_iter):
        """Targeted search with stronger boundary pressure and opposite-orientation pairing."""
        rng = random.Random(seed)
        nloc = len(centers)
        cur_c = [tuple(c) for c in centers]
        cur_a = list(angles)
        cur_c = repair_scale(cur_c, cur_a)
        cur_s = enclosing_side(cur_c, cur_a)
        best_c, best_a, best_s = cur_c[:], cur_a[:], cur_s

        step_xy = max(0.02, cur_s * 0.010)
        step_a = 0.20
        temp0 = 0.08
        ...
        for t in range(max_iter):
            ...
            # Bias moves toward walls and the opposite orientation motif.
            choice = rng.random()
            if choice < 0.35:
                dx = (rng.random() * 2.0 - 1.0) * step_xy
                dy = (rng.random() * 2.0 - 1.0) * step_xy
                if rng.random() < 0.45:
                    dx += math.copysign(step_xy * 0.75, old_c[0] if old_c[0] != 0 else (1 if rng.random() < 0.5 else -1))
                if rng.random() < 0.45:
                    dy += math.copysign(step_xy * 0.75, old_c[1] if old_c[1] != 0 else (1 if rng.random() < 0.5 else -1))
                cur_c[i] = (old_c[0] + dx, old_c[1] + dy)
            elif choice < 0.75:
                da = (rng.random() * 2.0 - 1.0) * step_a
                # Pull angles toward the two dominant opposite orientations.
                target = math.pi / 2.0 if ((i + t) & 1) == 0 else -math.pi / 2.0
                cur_a[i] = normalize_angle(old_a + 0.55 * da + 0.18 * normalize_angle(target - old_a))
            else:
                cur_a[i] = normalize_angle(old_a + math.pi + (rng.random() - 0.5) * 0.10)

            test_c = repair_scale(cur_c, cur_a)
            test_s = enclosing_side(test_c, cur_a)
            test_o = objective(test_c, cur_a, s_weight=4.0, overlap_weight=3500.0)

            accept = False
            if test_o <= objective(cur_c, cur_a, s_weight=4.0, overlap_weight=3500.0):
                accept = True
            else:
                temp = temp0 * (1.0 - t / max_iter) + 0.0015
                if rng.random() < math.exp(-(test_o - objective(cur_c, cur_a, s_weight=4.0, overlap_weight=3500.0)) / temp):
                    accept = True
    ...
    for seed in seeds:
        ...
        for mode in (0, 1, 2):
            ...
            centers, angles = best_permutation_reseed(centers, angles, 1000 + seed + 19 * mode)
            centers = repair_scale(centers, angles)

            # First stage: global stochastic improvement.
            centers, angles, s = local_search(
                centers, angles, seed=seed + 19 * mode, max_iter=2200 if n <= 12 else 1500
            )
            centers, angles, s = coordinate_descent(centers, angles, seed=seed + 19 * mode, rounds=4)

            # Second stage: stronger wall-biased anneal tends to improve square side.
            centers, angles, s2 = anneal_with_wall_bias(
                centers, angles, seed=7000 + seed + 97 * mode, max_iter=1800 if n <= 12 else 1200
            )
            if s2 < s:
                s = s2

            # Third stage: several deterministic angle-flip and translation refinements.
            for _ in range(3):
                changed = False
                for i in range(n):
                    c_try = centers[:]
                    a_try = angles[:]
                    a_try[i] = normalize_angle(a_try[i] + math.pi)
                    c_try = repair_scale(c_try, a_try)
                    s_try = enclosing_side(c_try, a_try)
                    if s_try < s:
                        centers, angles, s = c_try, a_try, s_try
                        changed = True

                    for da in (math.pi / 5.0, -math.pi / 5.0, 2.0 * math.pi / 5.0, -2.0 * math.pi / 5.0):
                        c_try = centers[:]
                        a_try = angles[:]
                        a_try[i] = normalize_angle(a_try[i] + da)
                        c_try = repair_scale(c_try, a_try)
                        s_try = enclosing_side(c_try, a_try)
                        if s_try < s:
                            centers, angles, s = c_try, a_try, s_try
    ...
    # Multi-start finish: alternate two optimizers and keep the best.
    for extra_seed in range(4):
        c1, a1, s1 = local_search(
            centers, angles, seed=9999 + n + extra_seed, max_iter=1100 if n <= 12 else 800
        )
        c2, a2, s2 = coordinate_descent(c1, a1, seed=4242 + n + extra_seed, rounds=5)
        c2 = repair_scale(c2, a2)
        s2 = enclosing_side(c2, a2)
        if s2 < s:
            centers, angles, s = c2, a2, s2

    # Final tightening with a very low-amplitude search.
    centers, angles, s = anneal_with_wall_bias(
        centers, angles, seed=88888 + n, max_iter=700 if n <= 12 else 500
    )
    centers = repair_scale(centers, angles)
    s = enclosing_side(centers, angles)
