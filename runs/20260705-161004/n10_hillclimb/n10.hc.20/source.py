...
def pack(n):
    ...
    def build_solution(xs):
        centers = []
        angles = []
        for i in range(n):
            centers.append((xs[3 * i], xs[3 * i + 1]))
            angles.append(normalize_angle(xs[3 * i + 2]))
        return centers, angles

    def pack_objective(xs):
        centers, angles = build_solution(xs)
        s = enclosing_side(centers, angles)
        pen = pairwise_overlap_penalty(centers, angles)
        sq = square_violation(centers, angles, s)
        # Strong preference for smaller squares, but allow infeasible exploration.
        return s + 80.0 * pen + 120.0 * sq

    def feasible_and_score(centers, angles):
        if has_overlap(centers, angles):
            return None
        return enclosing_side(centers, angles)

    # Deterministic seeds and motifs, with a stronger initial search for n=10.
    if n == 10:
    ...
    for base_seed in seeds:
        ...
        for mode in modes:
            ...
            centers, angles, s = local_search(
                ...
                max_iter=3000 if n <= 12 else 1800,
            )
            centers, angles, s = coordinate_descent(
                centers, angles, seed=base_seed + 97 * mode, rounds=6
            )

            if minimize is not None and n <= 14:
                x0 = []
                for (cx, cy), a in zip(centers, angles):
                    x0.extend([cx, cy, a])

                # Use a local derivative-free optimizer to tighten the best motif.
                # Soft constraints are handled in the objective; feasibility is checked after.
                res = minimize(
                    pack_objective,
                    x0,
                    method="Nelder-Mead",
                    options={
                        "maxiter": 2200 if n <= 10 else 1400,
                        "xatol": 1e-6,
                        "fatol": 1e-7,
                        "adaptive": True,
                    },
                ...
                if res is not None and res.x is not None:
                    centers2, angles2 = build_solution(res.x)
                    centers2, angles2, _ = coordinate_descent(
                        centers2, angles2, seed=base_seed + 193 * mode, rounds=2
                    )
                    centers2 = repair_scale(centers2, angles2)
                    s2 = feasible_and_score(centers2, angles2)
                    if s2 is not None and (best is None or s2 < best[2]):
                        best = (centers2[:], angles2[:], s2)

            centers = repair_scale(centers, angles)
            s = feasible_and_score(centers, angles)
    ...
    # Exact feasibility restoration and post-tightening.
    if has_overlap(centers, angles):
        centers = repair_scale(centers, angles)
    centers, angles, s = coordinate_descent(centers, angles, seed=4242 + n, rounds=5)
    centers = repair_scale(centers, angles)
    s = enclosing_side(centers, angles)

    # Final local numerical tightening using the current best as a warm start.
    if minimize is not None and n <= 14:
        x0 = []
        for (cx, cy), a in zip(centers, angles):
            x0.extend([cx, cy, a])
        ...
        if res is not None and res.x is not None:
            centers2, angles2 = build_solution(res.x)
            centers2 = repair_scale(centers2, angles2)
            if not has_overlap(centers2, angles2):
