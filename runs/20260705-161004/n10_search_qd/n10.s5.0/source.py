...
def _template_double_lattice(n, axial_shift=0.0, tilt=0.0, edge_tilt=0.0, xscale=1.0, yscale=1.0):
...
def _template_shell(n):
    ...
    if inner > 0:
        c2, a2 = _template_double_lattice(inner, axial_shift=0.38 * WIDTH, tilt=0.03, edge_tilt=0.06, xscale=1.0, yscale=1.0)
        centers.extend(c2)
        angles.extend(a2)
    return _center_pack(centers), angles
...
def _template_mixed(n):
    ...
    c1, a1 = _template_double_lattice(core_n, axial_shift=0.33 * WIDTH, tilt=0.02, edge_tilt=0.03, xscale=1.0, yscale=1.0)
    b = n - core_n
    c2, a2 = [], []
    if b > 0:
...
def _local_search(centers, angles, seed=0):
    ...
    lim = max(2.0, best_s * 1.4)
    bounds = []
    for _ in range(n):
        bounds.extend([(-lim, lim), (-lim, lim), (-math.pi, math.pi)])

    methods = [("Powell", 2000), ("Nelder-Mead", 1800)]
    for method, maxiter in methods:
    ...
    # Target the boundary first: move outer pieces toward the walls while re-centering.
    for phase in range(12):
        step_pos = max(0.0015, best_s * (0.020 - 0.0011 * phase))
        step_ang = max(0.012, 0.11 * (0.78 ** phase))
        edge_ids = sorted(range(n), key=lambda j: max(abs(best_centers[j][0]), abs(best_centers[j][1])), reverse=True)
        ids = edge_ids[: max(3, n // 2)]

        for _ in range(900):
            i = ids[rng.randrange(len(ids))]
            ...
            moves = [
                ...
                (-tx * step_pos, -ty * step_pos, 0.0),
                (ux * 0.5 * step_pos, uy * 0.5 * step_pos, step_ang),
                (ux * 0.5 * step_pos, uy * 0.5 * step_pos, -step_ang),
                (0.0, 0.0, step_ang),
                (0.0, 0.0, -step_ang),
                (ux * 0.9 * step_pos, uy * 0.9 * step_pos, 0.0),
                (0.0, 0.0, 0.0),
            ]
            ...
            for dx, dy, da in moves:
                ...
                if val is not None and val < best_local_s:
                    best_local = (cc, aa)
                    best_local_s = val

            if best_local is not None and best_local_s + 1e-12 < best_s:
                best_centers, best_angles, best_s = best_local
                continue

            if rng.random() < 0.18:
                dx, dy, da = moves[rng.randrange(len(moves))]
                cc = best_centers[:]
                aa = best_angles[:]
                cc[i] = (cx + dx, cy + dy)
                aa[i] = a + da
    ...
    # Final polishing with a compact vector optimization from the improved state.
    x0 = _vec_from_pack(best_centers, best_angles)
    lim = max(2.0, best_s * 1.25)
    bounds = []
    for _ in range(n):
        bounds.extend([(-lim, lim), (-lim, lim), (-math.pi, math.pi)])

    res = _safe_minimize(obj, x0, bounds=bounds, method="Powell", maxiter=2600)
    if res is not None:
        c, a = _pack_from_vec(res.x)
        if not has_overlap(c, a):
            s = enclosing_side(c, a)
...
def pack(n):
    ...
    templates.append(_template_double_lattice(n, axial_shift=0.27 * WIDTH, tilt=0.010, edge_tilt=0.090, xscale=0.96, yscale=0.98))
    templates.append(_template_double_lattice(n, axial_shift=0.45 * WIDTH, tilt=-0.045, edge_tilt=0.120, xscale=0.94, yscale=0.96))
