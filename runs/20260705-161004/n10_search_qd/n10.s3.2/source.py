...
def _candidate_templates(n):
    ...
    cands = []
    # More aggressive row-based variants with stronger boundary rows and tighter pitches.
    for rows in range(1, min(n, 6) + 1):
        row_lengths = _balanced_row_lengths(n, rows)
        if rows == 1:
            pitch_x = WIDTH * 0.76
            pitch_y = HEIGHT
        else:
            pitch_x = WIDTH * (0.73 + 0.025 * rows)
            pitch_y = HEIGHT * (0.66 + 0.03 * rows)
        cands.append(_template_rows(n, row_lengths, pitch_x, pitch_y, row_phase=0.0, tilt_edges=True))
        cands.append(_template_rows(n, row_lengths, pitch_x * 0.975, pitch_y * 1.015, row_phase=1.0, tilt_edges=True))
        cands.append(_template_rows(n, row_lengths, pitch_x * 0.95, pitch_y * 1.02, row_phase=0.5, tilt_edges=True))
...
def _local_search(centers, angles, seed=0):
    ...
    for phase in range(12):
        # Coordinate-wise and pairwise pattern moves.
        for _ in range(420):
            i = rng.randrange(len(best_centers))
            cx, cy = best_centers[i]
            a = best_angles[i]
            angsign = 1.0 if (i % 2 == 0) else -1.0
            ...
            moves = [
                ...
                (0.0, step_pos, 0.0),
                (0.0, -step_pos, 0.0),
                (0.8 * step_pos, 0.3 * step_pos, 0.0),
                (-0.8 * step_pos, -0.3 * step_pos, 0.0),
                (0.3 * step_pos, 0.8 * step_pos, 0.0),
                (-0.3 * step_pos, -0.8 * step_pos, 0.0),
                (0.0, 0.0, step_ang),
                (0.0, 0.0, -step_ang),
                (0.35 * step_pos, 0.0, step_ang * angsign),
                (-0.35 * step_pos, 0.0, -step_ang * angsign),
                (0.0, 0.35 * step_pos, -step_ang * angsign),
                (0.0, -0.35 * step_pos, step_ang * angsign),
            ]
            ...
            for dx, dy, da in moves:
                cc = best_centers[:]
                aa = best_angles[:]
                cc[i] = (cx + dx + bias_x * step_pos * 0.08, cy + dy + bias_y * step_pos * 0.08)
                aa[i] = a + da + (0.03 if (abs(cx) > abs(cy) and i % 2 == 0) else -0.03 if abs(cy) > abs(cx) and i % 2 == 1 else 0.0)
                cc = _center(cc)
                val = try_state(cc, aa)
                if val is not None and val + 1e-13 < best_s:
                    best_centers, best_angles, best_s = cc, aa, val
                    improved = True
                    break

            # Paired opposite-orientation move to encourage double-lattice motifs.
            if not improved and len(best_centers) >= 2 and rng.random() < 0.35:
                j = rng.randrange(len(best_centers) - 1)
                k = j + 1
                cc = best_centers[:]
                aa = best_angles[:]
                dx = rng.uniform(-step_pos, step_pos) * 0.5
                dy = rng.uniform(-step_pos, step_pos) * 0.5
                da = rng.uniform(-step_ang, step_ang) * 0.5
                cc[j] = (cc[j][0] + dx, cc[j][1] + dy)
                cc[k] = (cc[k][0] - dx, cc[k][1] - dy)
                aa[j] = aa[j] + da
                aa[k] = aa[k] - da
                cc = _center(cc)
                cc = repair(cc, aa)
                val = try_state(cc, aa)
                if val is not None and val <= best_s * 1.002:
                    best_centers, best_angles, best_s = cc, aa, val
                    improved = True

            # Boundary-push move: amplify edge utilization.
            if not improved and rng.random() < 0.20:
                cc = best_centers[:]
                aa = best_angles[:]
                edge_ids = [t for t, (x, y) in enumerate(cc) if abs(x) > abs(y)]
                if edge_ids:
                    j = rng.choice(edge_ids)
                    x, y = cc[j]
                    cc[j] = (x + math.copysign(step_pos * 0.18, x if abs(x) > 1e-10 else 1.0),
                             y + math.copysign(step_pos * 0.05, y if abs(y) > 1e-10 else 1.0))
                    aa[j] = aa[j] + (0.015 if x >= 0 else -0.015)
                    cc = _center(cc)
                    val = try_state(cc, aa)
                    if val is not None and val + 1e-13 < best_s:
                        best_centers, best_angles, best_s = cc, aa, val
    ...
    # Final deterministic tightening using pairwise translation sweep.
    changed = True
    while changed:
        changed = False
        for i in range(len(best_centers)):
            for j in range(i + 1, len(best_centers)):
                ...
                cc = _center(cc)
                val = try_state(cc, aa)
                if val is not None and val + 1e-14 < best_s:
                    best_centers, best_angles, best_s = cc, aa, val
                    changed = True
                    break
            if changed:
...
def pack(n):
    ...
    # Add a few randomized restarts around the strongest deterministic templates.
    expanded = []
    for c0, a0 in templates:
        expanded.append((c0, a0))
        if len(c0) >= 2:
            for _ in range(3):
                cc = [(x + random.uniform(-0.01, 0.01), y + random.uniform(-0.01, 0.01)) for x, y in c0]
                aa = [a + random.uniform(-0.03, 0.03) for a in a0]
                expanded.append((cc, aa))

    for seed, (c0, a0) in enumerate(expanded):
