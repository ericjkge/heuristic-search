...
def _candidate_templates(n):
    ...
    # Row-based packings with alternating orientations.
    for cols in range(1, min(n, 6) + 1):
        dx = WIDTH * (0.78 + 0.02 * cols)
        dy = HEIGHT * (0.70 + 0.025 * cols)
        cands.append(_grid_template(n, cols, dx, dy, phase=0, angle0=math.pi / 2.0, alt=True))
        cands.append(_grid_template(n, cols, dx * 0.98, dy * 1.01, phase=1, angle0=math.pi / 2.0, alt=True))
        cands.append(_grid_template(n, cols, dx * 1.02, dy * 0.97, phase=0, angle0=-math.pi / 2.0, alt=True))

    # Paired motifs with boundary tilts.
    for tilt in (0.0, math.pi / 28.0, math.pi / 20.0):
        cands.append(_pair_template(n, WIDTH * 0.83, HEIGHT * 0.77, tilt=tilt))

    # Dense zig-zag layouts.
    for cols in (2, 3, 4, 5):
        dx = WIDTH * (0.74 + 0.025 * cols)
        dy = HEIGHT * (0.66 + 0.03 * cols)
        centers, angles = [], []
        rows = int(math.ceil(n / cols))
        k = 0
        for r in range(rows):
            m = min(cols, n - k)
            y = (r - (rows - 1) / 2.0) * dy
            xshift = (0.5 * dx if r % 2 else 0.0) - (0.12 * dx if r % 3 == 2 else 0.0)
            x0 = -((m - 1) * dx) / 2.0 + xshift
            for c in range(m):
                x = x0 + c * dx
                ang = (math.pi / 2.0 if ((r + c) % 2 == 0) else -math.pi / 2.0)
                if r in (0, rows - 1):
                    ang += (math.pi / 26.0 if r == 0 else -math.pi / 26.0)
                if c == 0 or c == m - 1:
...
def _local_relax(centers, angles, seed=0, rounds=9):
    ...
    for phase in range(rounds):
        for _ in range(320):
            ...
            if not improved and rng.random() < 0.12:
                ...
                cc[j] = (
                    cc[j][0] + rng.uniform(-step_pos, step_pos) * 0.22,
                    cc[j][1] + rng.uniform(-step_pos, step_pos) * 0.22,
                )
                aa[j] = aa[j] + rng.uniform(-step_ang, step_ang) * 0.22
                cc = _center(cc)
                cc = repair(cc, aa)
                val = try_state(cc, aa)
...
def pack(n):
    ...
    for seed, (c0, a0) in enumerate(templates):
        c, a, s = _local_relax(c0, a0, seed=seed + 193, rounds=9)
