...
def _local_search(centers, angles, seed=0):
    ...
    res = _safe_minimize(obj, x0, bounds=bounds, method="Powell", maxiter=1400)
    if res is not None:
    ...
    # Coordinate-ish randomized improvement.
    step_pos = max(0.008, best_s * 0.016)
    step_ang = 0.10
        ...
        for _ in range(650):
            ...
            if phase >= 4:
                ...
                for j in edge_ids[: max(2, n // 3)]:
                    ...
                    aa[j] = aa[j] + (0.06 if j % 2 == 0 else -0.06)
                    cc = _center_pack(cc)
                    cand.append((cc, aa))
